...
from models import PASC_H
from struct_utils import load_comm_labels, compute_struct_grad, pretrain_psn
from load_data import load_data

def get_args():
    parser = argparse.ArgumentParser(description='PASC-H: Residual-Contrastive Twin Supervision')
    ...
    parser.add_argument('--loss_f_weight', type=float, default=default_config.get('loss_f_weight', 0.1))
    parser.add_argument('--loss_cf_weight', type=float, default=default_config.get('loss_cf_weight', 0.01))
...
def train(args, logger):
    ...
    train_data_full, val_data, test_data, adj = load_data(args.dataset, Data_file, args)
    ...
    comm_labels, twins_data, prior_score_net = load_comm_labels(args, comm_cache, adj, nodes, logger, train_data_full, X_prior.cpu().numpy(), Z_grad)
    ...
    train_edges_twins = twins_data[:, 2].astype(np.float32)
    train_edges_twins = (train_edges_twins > 0).astype(np.float32)
    twins_labels = torch.from_numpy(train_edges_twins).to(device)

    sw_treatment = torch.from_numpy(twins_data[:, 3].astype(np.float32)).to(device)
    logger.info(f"Structural twins: Twin Labels shape={twins_labels.shape}, Switched Treatment shape={sw_treatment.shape}")
    ...
    val_sw_t = 1 - (comm_labels[u_val] == comm_labels[v_val]).float()
    ...
    test_sw_t = 1 - (comm_labels[u_test] == comm_labels[v_test]).float()
    model = PASC_H(X_prior.shape[1], args.dim_h, args.dim_z, args.dropout, num_nodes=nodes).to(device)
    ...
    logger.info(f"Loss Configuration: Mode = [BCE]")
    ...
    try:
        for epoch in range(args.epochs):
            ...
            for perm in DataLoader(range(len(train_edges_tensor)), args.batch_size, shuffle=True):
                ...
                batch_f_lbl = train_labels[perm]
                batch_t_lbl = twins_labels[perm]
                batch_sw_t = sw_treatment[perm]
                ...
                optimizer.zero_grad()
                logits_fused, logits_original, logits_switched, logits_gate = model(
                    X_prior, batch_edges, T_fact, batch_sw_t,
                ...
                loss_fused = F.binary_cross_entropy_with_logits(logits_fused.squeeze(), batch_f_lbl)
                loss_original = F.binary_cross_entropy_with_logits(logits_original.squeeze(), batch_f_lbl)
                loss_switched = F.binary_cross_entropy_with_logits(logits_switched.squeeze(), batch_t_lbl)
                w_gate = torch.sigmoid(logits_gate)
                loss_gate = w_gate.mean()
                loss = loss_fused + args.loss_f_weight * loss_original + args.loss_cf_weight * loss_switched

                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
                optimizer.step()
                total_loss += loss.item() * len(perm)
                total_fused_loss += loss_fused.item() * len(perm)
                total_original_loss += loss_original.item() * len(perm)
                total_switched_loss += loss_switched.item() * len(perm)

            scheduler.step()
            avg_loss = total_loss / len(train_edges_tensor)
            avg_fused_loss = total_fused_loss / len(train_edges_tensor)
            avg_original_loss = total_original_loss / len(train_edges_tensor)
            avg_switched_loss = total_switched_loss / len(train_edges_tensor)
            model.eval()
            with torch.no_grad():
                u_val = val_edges_device[:, 0]
                v_val = val_edges_device[:, 1]
...
#  === Val ===
                logits_val, _, _, _ = model(
                    X_prior, val_edges_device, t_val, val_sw_t,
            ...
            logger.info(f"Ep {epoch+1}: Loss {avg_loss:.4f} Loss_orig {avg_original_loss:.4f} Loss_sw {avg_switched_loss:.4f} w_gate:{loss_gate.item():.4f} | Val AUC {val_auc:.4f} F1(b/m/u) {val_f1_bin:.4f}/{val_f1_macro:.4f}/{val_f1_micro:.4f} | Thresh {current_best_thresh:.4f} | {status}")
    ...
    with torch.no_grad():
        ...
        logits_test, _, _, logits_gate = model(
            X_prior, test_edges_device, t_test, test_sw_t,
        ...
        logger.info(f"F1 (Micro) : {test_acc:.4f}")
