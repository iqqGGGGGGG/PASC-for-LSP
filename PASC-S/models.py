...
class PASC_S(nn.Module):
    def __init__(
        ...
        input_dim_gate = dim_z * 4
        self.gate_net = nn.Sequential(
            nn.Linear(input_dim_gate, dim_z),
            nn.LayerNorm(dim_z),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(dim_z, 1)
    ...
    def forward(self, X_prior, edges, edge_index_pos, edge_index_neg, T, R_pos, R_neg):
        z_base = self.base_encoder(X_prior)
        z_context = self.ctx_agg(z_base, edge_index_pos, edge_index_neg, R_pos=R_pos, R_neg=R_neg)

        u, v = edges.T[0], edges.T[1]
        z_base_u, z_base_v = z_base[u], z_base[v]
        z_ctx_u, z_ctx_v = z_context[u], z_context[v]

        gate_input = torch.cat([z_base_u, z_base_v, z_ctx_u, z_ctx_v], dim=1)
        logits_gate = self.gate_net(gate_input)
        w = torch.sigmoid(logits_gate)
        z_fused_u = z_base_u + w * z_ctx_u
        z_fused_v = z_base_v + w * z_ctx_v
        logits_fused = self.decoder(z_fused_u, z_fused_v)

        return logits_fused, logits_gate
